...
from models import MISA, TemporalMISA, MMIMTemporalModel
...
CKPT_PATH     = os.path.join(PROJECT_ROOT, 'MISA', 'cv_results_v3.json')
...
def train_lr_baseline(X_all, y_all, train_idx, val_idx, fold_seed=42):
    ...
    print(f"    Fitting LR (C=1.0, l2, lbfgs, max_iter=1000)...", flush=True)
...
def train_static_mlp_baseline(X_all, y_all, train_idx, val_idx, fold_seed=42):
    ...
    print(f"    StaticMLP input_dim={input_dim}, max_epochs={MISA_MAX_EPOCHS}, patience={MISA_PATIENCE}", flush=True)
    ...
    for epoch in range(MISA_MAX_EPOCHS):
        ...
        print(f"    ep{epoch+1:02d} val_acc={acc:.4f} f1={f1:.4f} auc={auc:.4f}  best={best_acc:.4f} no_imp={no_improve}", flush=True)
        if no_improve >= MISA_PATIENCE:
            print(f"    [EarlyStop] ep{epoch+1} no_improve={no_improve}>={MISA_PATIENCE}", flush=True)
            break

    print(f"    [Done] best_acc={best_metrics[0]:.4f} f1={best_metrics[1]:.4f} auc={best_metrics[2]:.4f}", flush=True)
    return best_metrics
...
def train_baseline(model, train_loader, val_loader, modality='text', fold_seed=42):
    ...
    print(f"    Model params={n_params:,}  max_epochs={MAX_EPOCHS}  patience={PATIENCE}", flush=True)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if acc > best_acc:
            ...
            if no_improve >= PATIENCE:
                print(f"    [EarlyStop] ep{epoch+1} no_improve={no_improve}>={PATIENCE}", flush=True)
                break
...
def train_misa_variant(model, config, train_loader, val_loader, fold_seed=42):
    ...
    best_acc, best_metrics, no_improve = 0.0, (0., 0., 0.), 0
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"    Model params={n_params:,}  max_epochs={MISA_MAX_EPOCHS}  patience={MISA_PATIENCE}", flush=True)
...
def train_mmim_model(model, config, train_loader, val_loader, fold_seed=42, cpc_weight=0.1):
    """Train MMIMTemporalModel with task loss + CPC MI maximization auxiliary loss.

    Loss = CrossEntropy(task) + cpc_weight * CPC(Z→modalities)
    Much simpler than MISA: no diff_loss, no recon_loss, no adversarial discriminator.
    Reference: Han et al., EMNLP 2021, MultiModal InfoMax (MMIM).
    """
    set_seed(fold_seed)
    model = model.to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=MISA_WD)
    ...
    best_acc, best_metrics, no_improve = 0.0, (0., 0., 0.), 0
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"    Model params={n_params:,}  max_epochs={MISA_MAX_EPOCHS}  patience={MISA_PATIENCE}  cpc_w={cpc_weight}", flush=True)
    ...
    for epoch in range(MISA_MAX_EPOCHS):
        ...
        for batch in train_loader:
            ...
            logits    = outputs['future_risk']
            task_loss = criterion(logits, labels)
            cpc_l     = model.cpc_loss(outputs)
            loss      = task_loss + cpc_weight * cpc_l
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        scheduler.step()
...
def main():
    ...
    model_names = [
        ...
        'Concat', 'Weighted', 'Standard-MISA', 'HAD-M3H', 'MMIM-HAD',
        'wo-CF', 'wo-MC', 'wo-IM', 'wo-LSTM', 'wo-BF',
    ]
    if not ckpt:
        ckpt = {name: [] for name in model_names}
    else:
        # 补全新增模型的键（加载旧 checkpoint 后可能缺少新模型）
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        # ── MMIM-HAD ───────────────────────────────────────────────
        if not already_done('MMIM-HAD'):
            print(f"\n--- [F{fold_idx+1}] MMIM-HAD (MMIM fusion + temporal LSTM) ---", flush=True)
            cfg = Config()
            m   = MMIMTemporalModel(cfg)
            r   = train_mmim_model(m, cfg, train_loader, val_loader, fold_seed=fold_seed)
            ckpt['MMIM-HAD'].append(list(r)); save_ckpt(ckpt)
            print(f"  → acc={r[0]:.4f}  f1={r[1]:.4f}  auc={r[2]:.4f}", flush=True)
        else:
            print(f"  [CACHED] MMIM-HAD fold {fold_idx+1}", flush=True)
    ...
    baseline_models = ['RoBERTa-Mean+LR', 'RoBERTa-Mean+MLP',
                        'Text-only', 'Image-only', 'Behavior-only', 'Concat', 'Weighted', 'Standard-MISA', 'HAD-M3H', 'MMIM-HAD']
